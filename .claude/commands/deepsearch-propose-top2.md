...
- **Then search the CROSS-BRANCH tools** — ones that combine/span paradigms:
  ...
  frequently the tool that fits **no single bucket** — it is invisible to per-branch search
  **by construction.**
...
- Mine practitioner sources, not just first-page Google: **Reddit, Hacker News, GitHub
  `awesome-*` lists + topic search, Stack Overflow, survey papers, comparison blogs
...
- **Scrape ≥1 EXHAUSTIVE curated enumeration** — a GitHub `awesome-<topic>` list, a
  "comprehensive list of <category>" page, or a registry category index. These surface the
  long tail that per-branch queries and top-10 listicles structurally miss.
- Each round, issue NEW query angles — rename the branch, search siblings, add `reddit` /
  `github` / `2026` / `alternatives` qualifiers.
